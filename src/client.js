...
const transformFormToStateOptions = (form) => {
  ...
  return {
    ...
    colors: [formResults.gradientColor1, formResults.gradientColor2].map(
      hexToRgb
    ),
...
const createPreviewAnimation = (form, font, containerElement) => {
  ...
  replaceElement(
    containerElement,
    containerElement.children[0],
    initState.canvasFinal
  );
...
const renderGifHandler = (form, font, progressElement, linkContainer) => {
  ...
  const progressHandler = (progress) => {
    progressElement.textContent = `Combining frames: ${Math.floor(
      progress * 100
    )}%`;
  };
  ...
  const finishedHandler = (blob) => {
    ...
    const filename = document
      .getElementById("input-canvas-text")
      .value.replace(/[\\/:*?<>|. ]/g, "");
    downloadLink.download = filename;
    const sizeMB = Number.parseFloat(blob.size / 1024 / 1024);
    downloadLink.textContent = `Click to download GIF (${sizeMB.toPrecision(
      3
    )} MB)`;
...
const init = () => {
  ...
  Array.from(
    document.querySelectorAll(
      'form input[type="number"], form input[type="text"], form input[type="color"]'
    )
  ).forEach((element) => {
